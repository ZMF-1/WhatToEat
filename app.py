...
def hello_world():  # put application's code here
    if os.path.exists("days.wte") and os.path.isfile("days.wte"):
        with open("days.wte", 'r+', encoding='utf-8') as f:
            ...
            for i in range(len(liveddates)):
                liveddates[i] = liveddates[i].rstrip('\n')
...
def daily():
  ...
  <tr>
    ...
    """
    for j in range(1):
        i = datetime.date.weekday(datetime.date.today())
        if i == 0:
            wd = "星期一"
        elif i == 1:
            wd = "星期二"
    ...
    <td>{}</td>
        ...
        """.format(wd, menu[i][0], menu[i][1], menu[i][2], menu[i][3], menu[i][4], menu[i][5])
    tb += "</table>"
